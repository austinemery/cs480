#include "mycamera.h"

MyCamera::MyCamera()
{

}

MyCamera::~MyCamera()
{

}

bool MyCamera::Initialize(int w, int h)
{
  //--Init the view and projection matrices
  //  if you will be having a moving Mycamera the view matrix will need to more dynamic
  //  ...Like you should update it before you render more dynamic 
  //  for this project having them static will be fine
  view = glm::lookAt( glm::vec3(0.0, 8, -200.0), //Eye Position
                      glm::vec3(0.0, 0.0, 0.0), //Focus point
                      glm::vec3(0.0, 1.0, 0.0)); //Positive Y is up

  projection = glm::perspective( 45.0f, //the FoV typically 90 degrees is good which is what this is set to
                                 float(w)/float(h), //Aspect Ratio, so Circles stay Circular
                                 0.01f, //Distance to the near plane, normally a small value like this
                                 10000000000.0f); //Distance to the far plane, 
  return true;
}

glm::mat4 MyCamera::GetProjection()
{
  return projection;
}

glm::mat4 MyCamera::GetView()
{
  return view;
}

void MyCamera::updateCameraPosition(objectStruct objectData, glm::vec2 position)
{
  view = glm::lookAt( glm::vec3(position.x + 30, 30, position.y + 30), //Eye Position
                      glm::vec3(position.x, 0.0, position.y), //Focus point
                      glm::vec3(0.0, 1.0, 0.0));
}

void MyCamera::updateCameraPosition( glm::vec3 data)
{
  view = glm::lookAt( glm::vec3(data.x, data.y, data.z), //Eye Position
                      glm::vec3(0.0, 0.0, 0.0), //Focus point
                      glm::vec3(0.0, 0.0, -1.0));
}